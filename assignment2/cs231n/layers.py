...
def affine_backward(dout, cache):
    ...
    dw = x.reshape(x.shape[0], np.product(x.shape[1:])).T.dot(dout)
    db = dout.sum(axis=0)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def dropout_forward(x, dropout_param):
    ...
    if mode == 'train':
        ...
        #######################################################################
        mask = np.random.uniform(size=x.shape) > dropout_param['p']
...
def conv_backward_naive(dout, cache):
    ...
    for ih in range(Hout):
        ...
        for iv in range(Wout):
            ...
            dout_sub = dout[:, :, ih, iv]
            dw += dout_sub.T.dot(x_sub_reshaped)
            dx[:, :, hstart:hend, vstart:vend] += np.reshape(dout_sub.dot(w_reshaped), (N, C, HH, WW))
...
def max_pool_forward_naive(x, pool_param):
    ...
    for ih in range(Hout):
        ...
        for iv in range(Wout):
            vstart = iv * stride
            vend = vstart + pw
            out[:, :, ih, iv] = np.max(x[:, :, hstart:hend, vstart:vend], axis=(2, 3))
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    cache = (x, pool_param)
